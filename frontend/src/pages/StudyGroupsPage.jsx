...
import { Users, PlusCircle, LogIn, Key, ShieldCheck, MessageSquare, Phone, X, Trash2, Pencil } from 'lucide-react';
...
const StudyGroupsPage = () => {
    ...
    const handleCreate = async (e) => {
        ...
        try {
            const payload = { 
                GroupName: createForm.GroupName, Description: createForm.Description,
                Subject: createForm.GroupName, PhoneNumber: createForm.PhoneNumber,
                CreatedByEmail: userEmail, JoinCode: "", Members: [] 
            };
    ...
    const handleJoin = async (e) => {
        ...
        try {
            ...
            alert(err.response?.data?.message || "Invalid Code or Subject Mismatch!"); 
        }
    };
    ...
    const handleLeaveGroup = async (groupId) => {
        if (!window.confirm("Leave this hub?")) return;
        try {
    ...
    const handleUpdate = async (e) => {
        e.preventDefault();
        try {
            await axios.put(`${API_URL}/${editForm.id}`, {
                GroupName: editForm.GroupName, Description: editForm.Description, PhoneNumber: editForm.PhoneNumber
            }, { headers: { Authorization: `Bearer ${token}` } });
            setShowEditModal(false); fetchUserGroups(); alert("Updated!");
                            ...
                            {ownedGroups.map(g => (
                                <div key={g.id} onClick={() => navigate(`/chat/${g.id}`)} className="group p-5 rounded-3xl bg-indigo-50/50 border border-indigo-100 hover:border-indigo-400 cursor-pointer transition-all relative">
                                    ...
                                    <div className="flex items-center justify-between mt-3">
                                        <div className="flex items-center gap-2 p-2 bg-white rounded-xl shadow-sm border border-indigo-100 w-fit"><Key size={12} className="text-indigo-600"/><span className="text-xs font-black text-indigo-700 tracking-widest">{g.joinCode}</span></div>
                                        <div className="flex gap-2">
                                            <button onClick={(e) => { e.stopPropagation(); openEditModal(g); }} className="p-2 hover:bg-indigo-200 rounded-full text-indigo-600"><Pencil size={16}/></button>
                                            <button onClick={(e) => { e.stopPropagation(); handleDeleteGroup(g.id); }} className="p-2 hover:bg-red-100 rounded-full text-red-500"><Trash2 size={16}/></button>
                                        </div>
                                    </div>
                                </div>
                            ))}
                            ...
                            {joinedGroups.map(g => (
                                <div key={g.id} onClick={() => navigate(`/chat/${g.id}`)} className="group p-5 rounded-3xl bg-white border border-slate-100 hover:border-emerald-400 cursor-pointer transition-all shadow-sm">
                                    <div className="flex justify-between items-center">
                                        <div><h4 className="font-bold text-slate-700 text-sm">{g.groupName}</h4><p className="text-[9px] text-slate-400 mt-1 uppercase font-bold flex items-center gap-1"><Phone size={10}/> Admin: {g.phoneNumber}</p></div>
                                        <button onClick={(e) => { e.stopPropagation(); handleLeaveGroup(g.id); }} className="p-2 hover:bg-red-50 rounded-full text-red-400"><Trash2 size={16}/></button>
                                    </div>
                                </div>
                            ))}
                        </div>
            ...
            <main className="flex-1 p-10 overflow-y-auto bg-slate-50/20 text-center lg:text-left">
                <div className="max-w-5xl mx-auto space-y-12">
                    <div className="relative rounded-[3rem] overflow-hidden shadow-2xl h-[300px]">
                        <img src="https://images.unsplash.com/photo-1523240795612-9a054b0db644?auto=format&fit=crop&w=1200&q=80" className="absolute inset-0 w-full h-full object-cover" />
                        <div className="absolute inset-0 bg-indigo-900/90 flex flex-col justify-center px-12 text-white font-black uppercase italic tracking-tighter"><h1 className="text-5xl">Collaboration Hub</h1></div>
                    </div>
                    <div className="grid grid-cols-2 gap-10">
                        <button onClick={() => setShowCreateModal(true)} className="p-10 bg-white rounded-[3.5rem] shadow-xl border-b-8 border-b-indigo-500 text-left flex flex-col"><PlusCircle className="text-indigo-600 mb-6" size={40} /><h2 className="text-3xl font-black uppercase italic">Create Circle</h2></button>
                        <button onClick={() => setShowJoinModal(true)} className="p-10 bg-white rounded-[3.5rem] shadow-xl border-b-8 border-b-emerald-500 text-left flex flex-col"><LogIn className="text-emerald-600 mb-6" size={40} /><h2 className="text-3xl font-black uppercase italic">Join Peer</h2></button>
                    </div>
                </div>
            </main>
                    ...
                    <div className="bg-white w-full max-w-lg rounded-[3.5rem] p-12 relative shadow-2xl">
                        ...
                        <form onSubmit={handleJoin} className="space-y-4">
                            ...
                            <div className="pt-4 text-center">
                                ...
                                <input type="text" maxLength="6" required className="w-full px-5 py-6 bg-slate-50 border-4 border-slate-100 rounded-[2.5rem] text-center text-5xl font-black tracking-[1.2rem] text-emerald-600 focus:border-emerald-500 outline-none" placeholder="000000" onChange={e => setJoinForm({ ...joinForm, joinCode: e.target.value })} />
                            </div>
